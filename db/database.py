...
def init_db():
    ...
    # Archive May 18 legacy pre-OM signals (old loose scanner, before OM rules).
    try:
        archive_legacy_pre_om_signals()
    except Exception as e:
        logger.warning(f"archive_legacy_pre_om_signals failed at startup (non-fatal): {e}")
...
# Legacy pre-OM window: signals generated by the old loose scanner after the
# bad-run window ended, before OM rules were implemented. Om approved exclusion
# on 2026-05-18. Upper bound covers full May 18 UTC day.
LEGACY_PRE_OM_START = "2026-05-18 01:07:52"
LEGACY_PRE_OM_END   = "2026-05-19 00:00:00"
...
def archive_legacy_pre_om_signals(
    start_utc: str = LEGACY_PRE_OM_START,
    end_utc:   str = LEGACY_PRE_OM_END,
) -> int:
    """
    Soft-archive agent_signals generated by the old loose scanner on May 18 2026
    after the bad-run window closed (01:07:52 UTC) but before OM rules were
    implemented. These signals had entry_allowed=1 and passed market-hour guards,
    but were produced by loose legacy strategy logic — not OM-approved entries.

    Om approved exclusion on 2026-05-18.

    Marks rows:
      is_archived      = 1
      manual_exclusion = 1
      blocked_reason   = "legacy_pre_om_fake_signal"
      archive_reason   = "old_strategy_over_signaling_before_om_logic"
    ...
    if count:
        logger.info(
            f"archive_legacy_pre_om_signals: archived {count} signals "
            f"from {start_utc} → {end_utc} "
            f"(old_strategy_over_signaling_before_om_logic). "
            f"Rows kept in DB with is_archived=1, manual_exclusion=1."
        )
    return count
