...
- **In progress — 42 examples collected** (`docs/om_gold_scalp/OM_GOLD_SCALP_RULEBOOK.md`)
...
- Example 042: standalone 15M HTF-range breakdown bearish-continuation. Large purple range active — all internal moves are `no_trade_zone` + `avoid_entry`. Bearish continuation requires sequential: range low body close → retest holds below → follow-through. Failure = reclaim back inside range. `setup_type = htf_range_breakdown_bearish_continuation`. Audit fields: `htf_context`, `range_state`, `internal_pushes`, `avoid_short_before_break`, `avoid_long_inside_range`, `range_low_broken`, `range_retest_held_below`, `confirmation_signal`, `bearish_continuation_valid`, `execution_bias`, `failure_condition`
- Next planned: 043 — TBD by Om
